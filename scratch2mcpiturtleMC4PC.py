...
import gettext
import scratch
import mcpiext.minecraft as minecraft
import minecraftturtle
import minecraftstuff
import mcpiext.block as block
import time

VERSION = "1.0.3d for PC Multi"
...
#  mc = minecraft.Minecraft.create()
  ...
  for msg in _listen(s):
    if (msg):
      ...
      if msg[0] == 'broadcast':
        if not mc:
    	  mc = minecraft.Minecraft.create( server, 4711, playerName )
...
def main():
  ...
  argc = len(argvs)
  if (argc != 3):
	print 'Usgae: python %s MCPI-Server-IP playerName' % argvs[0]
	quit()

  server = argvs[1]
  playerName = argvs[2]
  print "Server IP Addr= %s " % argvs[1] 
  print "Player Name = %s " % argvs[2] 
  print "================="
